[{capture append="oxidBlock_content"}]
    [{if $oView->getInquirySendStatus() }]
        [{assign var="_statusMessage" value="PS_INQUIRYFORM_THANKYOU"|oxmultilangassign:$oxcmp_shop->oxshops__oxname->value}]
    ...
    <form class="js-oxValidate" action="[{ $oViewConf->getSslSelfLink() }]" method="post">
        <div>
            ...
            <input type="hidden" name="cl" value="psinquiryform_view"/>
        ...
        <ul class="form clear">
            ...
            <li [{if $aErrors.oxuser__oxlname}]class="oxInValid"[{/if}]>
                <label class="">[{ oxmultilang ident="LAST_NAME" }]</label>
                <input type="text" name="editval[oxuser__oxlname]" size=70 maxlength=40 value="[{if $oxcmp_user && !$editval.oxuser__oxlname}][{$oxcmp_user->oxuser__oxlname->value}][{else}][{$editval.oxuser__oxlname}][{/if}]" class="">
            </li>
            ...
            <li>
                <label>[{ oxmultilang ident="PS_INQUIRYFORM_PRODUCTS" suffix="COLON" }]</label>
                <textarea rows="15" cols="70" name="c_message" class="areabox">[{$oView->getInquiryMessage()}]</textarea>
            </li>
            <li class="verify">
